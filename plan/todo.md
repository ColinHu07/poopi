...
- [ ] **P0 · REVIEW-07 — Recover, delete, and redo personal ratings** — Owner: `@codex` · Depends on: REVIEW-04, REVIEW-06 · Done when: Profile keeps user-created bathrooms reachable even if their first review was never saved, every saved rating has a small delete control with confirmation, deletion removes its labels/note and refreshes public summaries, and the bathroom can be rated again. PR: pending
...
- [ ] **P0 · UX-02 — Make rating a one-tap core action** — Owner: `@codex` · Depends on: REVIEW-02 · Done when: the middle tab is a prominent accessible plus button that opens the rating flow, and all alphabetized good/bad labels appear as direct checkboxes after the overall rating instead of behind another modal. PR: pending
