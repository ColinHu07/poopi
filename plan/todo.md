...
- [x] **P0 · REVIEW-06 — Reopen and edit personal reviews** — Owner: `@codex` · Depends on: REVIEW-02, REVIEW-03 · Done when: bathroom details detect the signed-in user's prior review, Profile keeps a bathroom diary, reopening a review preloads every score/label/note/access/status/privacy field, and saving updates the same visit instead of creating a duplicate. PR: [#17](https://github.com/ColinHu07/poopi/pull/17)
- [x] **P0 · REVIEW-07 — Recover, delete, and redo personal ratings** — Owner: `@codex` · Depends on: REVIEW-04, REVIEW-06 · Done when: Profile keeps user-created bathrooms reachable even if their first review was never saved, every saved rating has a small delete control with confirmation, deletion removes its labels/note and refreshes public summaries, and the bathroom can be rated again. PR: [#18](https://github.com/ColinHu07/poopi/pull/18)
- [x] **P0 · UX-01 — Establish Poopi's playful visual system** — Owner: `@codex` · Depends on: MAP-03, REVIEW-02 · Done when: the map, result cards, details, rating flow, auth, profile, lists, feed, ranking, and navigation use one responsive, accessible warm-paper/cocoa/jade/coral design system derived from an approved visual direction. PR: [#17](https://github.com/ColinHu07/poopi/pull/17)
- [x] **P0 · UX-02 — Make rating a one-tap core action** — Owner: `@codex` · Depends on: REVIEW-02 · Done when: the middle tab is a prominent accessible plus button that opens the rating flow, and all alphabetized good/bad labels appear as direct checkboxes after the overall rating instead of behind another modal. PR: [#18](https://github.com/ColinHu07/poopi/pull/18)
...
- [ ] **P1 · SOCIAL-05 — Finish meaningful pairwise ranking** — Owner: `@codex` · Depends on: REVIEW-02 · Done when: pair selection avoids repeated questions, every identity has one mutable opinion per pair, the weighted community Bradley-Terry order updates deterministically, interpolated scores include confidence context, and personal ordering remains a separately labeled optional view. Review-aware matching and seeding: [#21](https://github.com/ColinHu07/poopi/pull/21); compare-before-ranking insertion and stable personal-score display: [#22](https://github.com/ColinHu07/poopi/pull/22); score confidence UI remains.
